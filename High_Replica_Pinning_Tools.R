...
#
matrix_preparation <- function(file_lib){
  ...
  # Data library
  df_lib <- read_csv(file_lib, show_col_types = FALSE) %>%
...
merging_files <- function(file_lib, data_gene, cells = 1536) {
...
# a screen pattern and select the plate letter
# @data: Data frame. Screen data that contain the colony areas. Direct input from 
# @Colony_areas function.
# @screen_pattern: Character. It can be used "YKO" or "ts"
# @letter: Character. Select a letter that describe the screenning like "A" or "B"
Screen_processing <- function(data, screen_pattern, letter) {
  df <- data %>% 
